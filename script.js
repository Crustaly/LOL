...
// Hardcoded API key that doesn't expire
const HARDCODED_API_KEY = 'RGAPI-85f2a440-debe-4931-bd7f-cfb521126bdd';

// User credentials storage
let userCredentials = {
  apiKey: HARDCODED_API_KEY, // Hardcoded API key (doesn't expire)
  gameName: null,
  tagLine: null,
  region: null
};
  ...
  const loadPromise = (async () => {
    try {
      if (showLoading) {
        ...
        const apiKeyToUse = userCredentials.apiKey || HARDCODED_API_KEY;
...
function handleLoginSubmit(e) {
  ...
  // Use hardcoded API key (no longer needed from form)
  ...
  // Validate required fields (API key is now hardcoded, so no need to validate it)
  ...
  // Save credentials (always use hardcoded API key)
  saveCredentials(apiKey, gameName, tagLine, region);
